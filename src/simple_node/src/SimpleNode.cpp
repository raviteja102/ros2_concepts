...
SimpleNode::SimpleNode() : Node("simple_node") {
  RCLCPP_INFO(this->get_logger(), "Constructor for the Simple Node initialize "
                                  "the key elements needed for the node");
  ...
  timer_ = this->create_wall_timer(std::chrono::milliseconds(time_interval_),
                                   std::bind(&SimpleNode::timerCallback, this));
}

void SimpleNode::subscriberCallback(
    const std_msgs::msg::String::SharedPtr msg) {
  RCLCPP_INFO(
      this->get_logger(),
      "Message received from publisher and here is message and counter %s  %d",
      msg->data.c_str(), ++counter_);
}
